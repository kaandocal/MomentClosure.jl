...
κ = MomentClosure.cumulants_to_central_moments(Moment{2}, 4)
@test isequal(κ[1,2], M[1,2])
expr = M[3,1] - 3*M[1,1]*M[2,0]
@test isequal(κ[3,1], expr)

μ = define_μ(Moment{2}, 3)
κ = MomentClosure.cumulants_to_raw_moments(Moment{2},3)
@test isequal(κ[1,0], μ[1,0])
expr = μ[2,1] + μ[0,1]*(2*μ[1,0]^2 - μ[2,0]) - 2*μ[1,0]*μ[1,1]
# if this one fails most likely simplification functions have broke
@test isequal(κ[2,1],expr)

raw_to_central = MomentClosure.raw_to_central_moments(Moment{2},3)
...
central_to_raw = MomentClosure.central_to_raw_moments(Moment{2},3)
expr = M[2,0] + μ[1,0]^2
